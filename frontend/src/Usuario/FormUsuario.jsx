import React, { useState, useContext, useEffect } from "react";
import usePostRequest from "../Hooks/usePostRequest";
import { AlertContext } from "../context/AlertContext";
import { BeatLoader, ClockLoader } from "react-spinners";
import useSubmitForm from "./useSubmitForm";
import { useNavigate } from "react-router-dom";

const FormUsuario = ({
  nombre,
  setNombre,
  apellido,
  setApellido,
  password,
  setPassword,
  correo,
  setCorreo
}) => {
  const [passwordError, setPasswordError] = useState(""); // Estado para el mensaje de error
  const [correoError, setCorreoError] = useState(""); // Estado para el mensaje de error
  const [showPassword, setShowPassword] = useState(false);

  // Función para validar la contraseña
  const validatePassword = (value) => {
    // Verifica si se ha ingresado una contraseña
    if (value.trim() !== "") {
      const regex = /^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[@$!%*?&\\\/])[\s\S]{8,}$/;
      if (!value.match(regex)) {
        setPasswordError("La contraseña debe tener al menos 8 caracteres, incluyendo al menos una letra mayúscula, un número y un carácter especial.");
      } else {
        setPasswordError("");
      }
    } else {
      // Si no se ingresó contraseña, no muestra ningún error
      setPasswordError("");
    }
  };

  // Función para validar el correo electrónico
  const validateEmail = (value) => {
    // Verifica si se ha ingresado un correo electrónico
    if (value.trim() !== "") {
      const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!value.match(regex)) {
        setCorreoError("Por favor, introduce una dirección de correo electrónico válida.");
      } else {
        setCorreoError("");
      }
    } else {
      // Si no se ingresó correo electrónico, no muestra ningún error
      setCorreoError("");
    }
  };

  const togglePasswordVisibility = () => {
    setShowPassword(!showPassword);
  }
  return (
    <>
      <div className="col-md-6">
        <div className="form-floating mt-2 g-2">
          <input
            type="text"
            className="form-control"
            value={nombre}
            onChange={(e) => setNombre(e.target.value)}
            required
          />
          <label htmlFor="floatingSelect">Nombre:</label>
        </div>
      </div>
      <div className="col-md-6">
        <div className="form-floating mt-2 g-2">
          <input
            type="text"
            className="form-control"
            id="apellido"
            value={apellido}
            onChange={(e) => setApellido(e.target.value)}
            required
          />
          <label htmlFor="floatingSelect">Apellido:</label>
        </div>
      </div>
      <div className="col-md-6">
        <div className="form-floating mt-2 g-2 position-relative">
          <input
            type={showPassword ? "text" : "password"} // Mostrar la contraseña si showPassword es true
            className="form-control"
            id="password"
            value={password}
            // required
            onChange={(e) => {
              setPassword(e.target.value);
              validatePassword(e.target.value); // Validación de contraseña
            }}
          />
          <label htmlFor="floatingSelect">Password:</label>
          <i                  
            className="bi bi-eye position-absolute top-50 translate-middle-y"
            onClick={togglePasswordVisibility}
            style={{ cursor: 'pointer', right: '10px' }} // Agregar margen derecho de 10px
            >  
            {showPassword ? 
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-eye-slash" viewBox="0 0 16 16">
            <path d="M13.359 11.238C15.06 9.72 16 8 16 8s-3-5.5-8-5.5a7 7 0 0 0-2.79.588l.77.771A6 6 0 0 1 8 3.5c2.12 0 3.879 1.168 5.168 2.457A13 13 0 0 1 14.828 8q-.086.13-.195.288c-.335.48-.83 1.12-1.465 1.755q-.247.248-.517.486z"/>
            <path d="M11.297 9.176a3.5 3.5 0 0 0-4.474-4.474l.823.823a2.5 2.5 0 0 1 2.829 2.829zm-2.943 1.299.822.822a3.5 3.5 0 0 1-4.474-4.474l.823.823a2.5 2.5 0 0 0 2.829 2.829"/>
            <path d="M3.35 5.47q-.27.24-.518.487A13 13 0 0 0 1.172 8l.195.288c.335.48.83 1.12 1.465 1.755C4.121 11.332 5.881 12.5 8 12.5c.716 0 1.39-.133 2.02-.36l.77.772A7 7 0 0 1 8 13.5C3 13.5 0 8 0 8s.939-1.721 2.641-3.238l.708.709zm10.296 8.884-12-12 .708-.708 12 12z"/>
            </svg> : 
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-eye" viewBox="0 0 16 16">
              <path d="M16 8s-3-5.5-8-5.5S0 8 0 8s3 5.5 8 5.5S16 8 16 8M1.173 8a13 13 0 0 1 1.66-2.043C4.12 4.668 5.88 3.5 8 3.5s3.879 1.168 5.168 2.457A13 13 0 0 1 14.828 8q-.086.13-.195.288c-.335.48-.83 1.12-1.465 1.755C11.879 11.332 10.119 12.5 8 12.5s-3.879-1.168-5.168-2.457A13 13 0 0 1 1.172 8z"/>
              <path d="M8 5.5a2.5 2.5 0 1 0 0 5 2.5 2.5 0 0 0 0-5M4.5 8a3.5 3.5 0 1 1 7 0 3.5 3.5 0 0 1-7 0"/>
            </svg>
            }
          </i>
        </div>
        {passwordError && (
          <div className="text-danger" style={{fontSize: 12}}>{passwordError}</div>
        )}
      </div>
      <div className="col-md-6">
        <div className="form-floating mt-2 g-2">
          <input
            type="text"
            className="form-control"
            id="correo"
            value={correo}
            onChange={(e) => {
              setCorreo(e.target.value);
              validateEmail(e.target.value);
            }}
          />
          <label htmlFor="floatingSelect">Correo Electronico:</label>
          {correoError && (
              <div className="text-danger" style={{fontSize: 12}}>{correoError}</div>
            )}
        </div>
      </div>
      {/* <div className="col-12">
        <div className="form-floating">
          <select
            className="form-select" id="floatingSelect"
            value={rol}
            onChange={(e) => setRol(Array.from(e.target.selectedOptions, option => option.value))}
            multiple
          >
            <option value="Administrador">Administrador</option>
            <option value="Secretaria">Secretaria</option>
            <option value="Dea">Dea</option>
          </select>
          <label htmlFor="rol">Roles:</label>
          <small className="text-muted">Seleccione uno o más roles</small>
        </div>
        </div> */}
      
    </>
  );
};
export default FormUsuario;
